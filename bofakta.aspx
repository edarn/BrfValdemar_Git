<%@ Page Title="" Language="C#" MasterPageFile="~/SiteMaster1.master" AutoEventWireup="true"
    CodeFile="bofakta.aspx.cs" Inherits="bofakta" %>

<asp:Content ID="Content1" ContentPlaceHolderID="HeadContent" runat="Server">
</asp:Content>
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="Server">
    <%-- <table align="center" border="0" with="100%">
        <tr>
            <td align="center" width="100%">
                <a href="inform.html" target="_self">Tillbaka till information</a>
            </td>
        </tr>
    </table>
    <br>--%>
    <table border="0" with="100%" align="center">
        <tr>
            <td width="100%">
                <a href="javascript:history.go(-1);">Tillbaka till information</a>
            </td>
        </tr>
    </table>
    <br>
    <table width="100%" class="border1">
        <tr>
            <td style='background: #efefef; font-weight: bold; text-align: center;'>
                Bofakta om våra lägenheter
            </td>
        </tr>
        <tr>
            <td width="100%">
                <i>
                    <br>
                    OBS! Så här var lägenheterna utrustade när de var nya 1989/90
                    <br />
                </i><b>Kök med matplats<br>
                </b>Spis med 4 kokplattor, löstagbart hällskydd, tippskydd och luckspärr. Ugn med
                grill och värmeskåp. Spiskåpa med belysning. Kyl och svalskåp, frysskåp – allt i
                vitt. Plats för diskmaskin finns och förberedelse för anslutning är klara. 3 skift
                kakel som stänkskydd över bänkinredning. Belysning under väggskåp och tak.
                <br />
                <br />
                <b>Wc/Dusch/Badrum<br>
                </b>Wc-stol, tvättställ, badkar, badrumsskåp med spegel. Handukshängare, stödhantag
                vid badkar, klädkrok och toalettpappershållare.<br>
                Väggytor i 4 rok kakelsatt från golv till tak vid badkar, i övrigt 8 skifte eller
                målad väv.<br>
                Väggytor 2 rok och 3 rok kakelsatt från golv till tak vid badkar tvättställ och
                Wc-stol, i övrigt målad väv.<br>
                Helsvetsad plastmatta på golv.<br />
                <br />
                <b>Tvätt<br>
                </b>Tvättbänk, tvättmaskin och torkskåp. Väggskåp. Målade väggar. Plastmatta på
                golv.<br />
                <br />
                <b>Kapprum/Hall<br>
                </b>Tapetserade väggar. Hatthylla och skåp. Golv kan variera.<br />
                <br />
                <b>Vardagsrum<br>
                </b>Tapetserade väggar. Skåp enligt ritning. Variation av golv kan förekomma.<br />
                <br />
                <b>Förråd<br>
                </b>Till varje lägenhet finns det ett yttre kallförråd. Tvåorna och treorna har
                en stor klädkammare i lägenheten. Fyrorna har en liten klädkammare. Tvåorna och
                fyrorna har även ett mindre förråd i gemensamt utrymme.<br />
                <br />
                <b>Övrigt<br>
                </b>Centralantenn ansluten till kabel-tv, uttag för radio och TV samt telefon.
                <br />
                Fönsterbänkar. Betogplattor vid entré och uteplats. 3 rok med veranda har dock uteplats
                vid entrén.<br />
                <br />
                <b>Uppvärmning och ventilation<br>
                </b>Uppvärmning sker via fjärrvärmekulvert. Värmesystemet i lägenheterna är vattenburet.
                Alla lägenheter är utrustade med mekanisk till och från luftsventilation med värmeåtervinning.
                Luften som sugs ut från kök och våtutrymmen passerar genom en värmeväxlare där friskluften
                som tillförs lägenheten förvärms.<br />
            </td>
        </tr>
    </table>
</asp:Content>
